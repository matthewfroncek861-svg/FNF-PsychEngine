...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		portrait = new FlxSprite().loadGraphic(Paths.image('freeplayPorts/P1RonaldMenu'));
		...
		for (i in 0...songs.length)
		{
			...
			songText.screenCenter(X);
			portrait.loadGraphic(Paths.image('freeplayPorts/'+songs[i].songName.toLowerCase()));// it would be funny if this actually worked
		}

		scoreText = new FlxText(FlxG.width * 0.7, 5, 0, "", 32);
		scoreText.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, RIGHT);

		scoreBG = new FlxSprite(scoreText.x - 13, 0).makeGraphic(1, 66, 0xFF000000);
		scoreBG.alpha = 0.6;
		add(scoreBG);
	...
	private function updateportrait() {
		portrait.loadGraphic(Paths.image('freeplayPorts/'+songs[curSelected].songName.toLowerCase()));
		portrait.scale.set(0.5,0.5);
		portrait.updateHitbox();
		portrait.screenCenter(XY);
